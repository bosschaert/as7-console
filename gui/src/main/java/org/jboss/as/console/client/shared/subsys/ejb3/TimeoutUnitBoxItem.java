 ...
 */
public class TimeoutUnitBoxItem extends FormItem<TimeoutValue> implements ChoiceItem<String> {
    private static final BeanFactory factory = GWT.create(BeanFactory.class);

    private boolean modified = false; // TODO not needed if we make super.setModified() protected
    private final Class<TimeoutValue> valueClass;
    ...
    public TimeoutUnitBoxItem(String name, String unitName, String title, Class<TimeoutValue> cls) {
        super(name, title);
    ...
    @Override
    public TimeoutValue getValue() {
        String textValue = textBox.getValue();
        String unitValue = getUnitValue();
        TimeoutValue value = factory.poolTimeoutUnits().as();
        value.setValue(Long.parseLong(textValue));
    ...
    @Override
    public void setValue(TimeoutValue value) {
        textBox.setValue("" + value.getValue());
        setUnitValue(value.getUnitValue());
    }

    private void setUnitValue(String value) {
        int idx = -1;
    ...
    */
    @Override
    public boolean validate(TimeoutValue value) {
        return true;
    }
